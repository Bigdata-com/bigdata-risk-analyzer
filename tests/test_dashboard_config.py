...
def test_at_a_glance_config_extraction():
    ...
    def extract_config(current_config):
        ...
        return {
            "theme": current_config.get("main_theme") or current_config.get("theme") or "N/A",
...
def test_dashboard_cards_js_content():
    ...
    assert "Risk Scenario:" in content, (
        "Should say 'Risk Scenario:' not 'Theme:'"
    )
    
    # Check for risk-appropriate colors
    assert "text-orange-400" in content, (
        "Should use orange color for risk factors"
    )
    
    assert "text-red-400" in content, (
        "Should use red color for highest score"
    )
    
    assert "border-orange-600" in content, (
        "Should use orange border for risk context"
    )
...
def test_form_js_updates_config_badge():
    ...
    form_js_file = Path(__file__).parent.parent / "bigdata_risk_analyzer" / "static" / "scripts" / "form.js"
...
def test_color_scheme_consistency():
    ...
    base_path = Path(__file__).parent.parent / "bigdata_risk_analyzer" / "static" / "scripts"
...
def test_live_analysis_data_structure():
    ...
    live_data = {
        ...
        "content": [
            {"company": "Microsoft Corp.", "text": "Sample evidence"}
        ]
    }
