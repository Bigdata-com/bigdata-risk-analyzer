...
def test_hierarchical_taxonomy_structure():
    ...
    for child in taxonomy['children']:
        ...
        print(f"   - Category: '{child['label']}' (node {child['node']}, {len(child['children'])} risks)")
    
    print(f"✅ Hierarchical taxonomy structure is valid")
...
def test_taxonomy_not_converted_by_adapter():
    ...
    report_renderer_file = Path(__file__).parent.parent / "bigdata_risk_analyzer" / "static" / "scripts" / "report_renderer.js"
...
def test_mindmap_expects_hierarchical_structure():
    ...
    mindmap_file = Path(__file__).parent.parent / "bigdata_risk_analyzer" / "static" / "scripts" / "mindmap.js"
    ...
    print(f"✅ mindmap.js expects hierarchical structure with node IDs")
...
def test_all_demo_taxonomies_compatible():
    ...
    for filename, expected_type in files.items():
        ...
        if expected_type == 'hierarchical':
            assert 'children' in taxonomy, f"{filename} should have hierarchical taxonomy"
            assert 'node' in taxonomy, f"{filename} should have node IDs"
            assert isinstance(taxonomy.get('children'), list), f"{filename} children must be list"
            print(f"✅ {filename} has hierarchical taxonomy (root: '{taxonomy.get('label', 'N/A')}')")
        else:
...
def test_adapter_preserves_taxonomy_fields():
    ...
    print(f"✅ Adapter preserves all taxonomy fields")
