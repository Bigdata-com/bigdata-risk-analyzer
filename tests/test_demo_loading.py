...
def test_all_json_files_have_required_structure():
    ...
    for filename, requirements in files.items():
        ...
        print(
            f"✅ {filename}: {len(companies)} companies, {len(content)} content items"
        )
...
def test_company_data_structure():
    ...
    for filename in [
        ...
        assert "composite_score" in sample_company, (
            f"{filename} company missing 'composite_score'"
        )
...
def test_content_data_structure():
    ...
    for filename in [
        ...
        for field in required_fields:
            assert field in sample_content, f"{filename} content item missing '{field}'"
...
def test_config_panel_templates_match():
    ...
    for template in required_templates:
        assert template in content, f"config_panel.js missing template: {template}"
    ...
    print("✅ config_panel.js has correct file paths")
...
def test_taxonomy_conversion_logic():
    ...
    assert "children" in taxonomy, (
        "import_tariffs.json should have hierarchical taxonomy"
    )
    ...
    print("✅ Hierarchical taxonomy structure validated")
...
def test_demo_metadata_display():
    ...
    print("✅ Dashboard cards show correct demo metadata")
    print("   - Source: News")
    print("   - Period: Last Month")
    print("   - Analysis generated on 15/10/2025")
